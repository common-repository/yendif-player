...
class Yendif_Featured_Videos {
	...
	public function build_gallery( $attributes ) {
		...
		$sql  = "SELECT * FROM $table WHERE published=1 AND type!='audio' AND featured=1";			
	 ...
	 * @since     1.2.0
	 *
	 * @return      string		Page URL to load other featured videos
	 */
	private function get_more_permalink() {
		
		global $wpdb;
		
		$post_id = $wpdb->get_var( "select ID from ".$wpdb->prefix."posts WHERE post_type='videoplaylist' AND post_content LIKE '%[yendifgallery featured=1]%' LIMIT 1" );
