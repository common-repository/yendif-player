...
class Yendif_Videos_Widget extends WP_Widget {	
	...
	public function __construct() {
		parent::__construct(
			'yendif-videos',
			__('Yendif Playlist Videos', YENDIF_PLAYER_PLUGIN_SLUG),
			array( 'description' => __('Use this widget to show Yendif Playlist Videos in your website\'s sidebar.', YENDIF_PLAYER_PLUGIN_SLUG) )
		);
	}
	...
	public function form( $instance ) {
		$defaults = array( 
			'title'             =>  __('Videos', YENDIF_PLAYER_PLUGIN_SLUG),
			'playlist'          => '-1',			
			'sort'         	    => 'latest',
			'thumb_width'       => 145,
			'thumb_height'      => 80,
    ...
    <p>
  	  <label for="<?php echo $this->get_field_id( 'playlist' ); ?>"><?php _e('Playlist : [optional]', YENDIF_PLAYER_PLUGIN_SLUG); ?></label>
      <?php  	
      	global $wpdb;		
		$table = $wpdb->prefix . 'yendif_player_playlists';
		$sql = "SELECT id, name FROM $table WHERE published=1";
		$_items = $wpdb->get_results($sql);
	  ?>
      <select class="widefat" id="<?php echo $this->get_field_id('playlist');?>" name="<?php echo $this->get_field_name('playlist'); ?>">
        <option value="-1">- <?php _e('Select playlist', YENDIF_PLAYER_PLUGIN_SLUG); ?> -</option>
        <?php
		  foreach ( $_items as $_item) {
			echo "<option ".($_item->id == $instance['playlist'] ? 'selected ' : '')." value=".$_item->id.">".$_item->name."</option>\n";							
		  } 
		?>
      </select>       
	</p> 
